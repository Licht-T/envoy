  ...
  std::string mysqlSslConfig() {
    return fmt::format(
        fmt::runtime(TestEnvironment::readFileToStringForTest(TestEnvironment::runfilesPath(
            "contrib/mysql_proxy/filters/network/test/mysql_ssl_test_config.yaml"))),
...
public:
  ...
  void initialize() override {
    ...
    tls_context_ =
        Ssl::createClientSslTransportSocketFactory({}, *tls_context_manager_, *api_);
...
TEST_P(MySQLSSLIntegrationTest, CachingSha2FastAuth) {
  ...
  ASSERT_TRUE(fake_upstream->waitForData(
      [](const std::string& data) { return data.length() > 0; }, &upstream_data));
...
TEST_P(MySQLSSLIntegrationTest, CachingSha2FullAuthRsaMediation) {
  ...
  uint32_t enc_len = static_cast<uint8_t>(enc_pkt[0]) |
                     (static_cast<uint8_t>(enc_pkt[1]) << 8) |
