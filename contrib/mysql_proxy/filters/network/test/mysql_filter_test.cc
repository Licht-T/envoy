 ...
 * SM: greeting(p=10) -> challenge-req(v41) -> serv-resp-more -> (data exchange) -> Req
 */
TEST_F(MySQLFilterTest, MySqlHandshake41AuthMoreTest) {
  ...
  Buffer::OwnedImpl credentials("!@#$creds$#@!");
  BufferHelper::encodeHdr(credentials, CHALLENGE_SEQ_NUM + 1);
  EXPECT_EQ(Envoy::Network::FilterStatus::Continue, filter_->onData(credentials, false));
  EXPECT_EQ(MySQLSession::State::AuthSwitchMore, filter_->getSession().getState());

  std::string srv_resp_ok_data = encodeClientLoginResp(MYSQL_RESP_OK, 0, CHALLENGE_SEQ_NUM + 2);
