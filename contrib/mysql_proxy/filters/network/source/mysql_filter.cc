...
Network::FilterStatus MySQLFilter::onData(Buffer::Instance& data, bool) {
  ...
  if (rsa_auth_state_ == RsaAuthState::WaitingClientPassword && !cleartext_password_.empty()) {
    data.drain(data.length());

    uint8_t inject_seq = getSession().getExpectedSeq(false) - 1;
    ENVOY_CONN_LOG(trace, "mysql_proxy: intercepted client password, sending request-public-key (seq={})",
...
Network::FilterStatus MySQLFilter::onWrite(Buffer::Instance& data, bool) {
  ...
  if (rsa_auth_state_ == RsaAuthState::WaitingServerKey) {
    ...
    if (BufferHelper::peekHdr(write_buffer_, len, seq) != DecodeStatus::Success ||
      ...
      ENVOY_CONN_LOG(trace, "mysql_proxy: waiting for complete public key packet ({} bytes buffered)",
...
void MySQLFilter::doRewrite(Buffer::Instance& data, uint64_t remaining, bool is_upstream) {
  ...
  ASSERT(data.length() == original_data_size,
         "doRewrite must not change overall buffer size");
}
...
void MySQLFilter::onServerGreeting(ServerGreeting& greeting) {
  ENVOY_CONN_LOG(trace, "mysql_proxy: server greeting: version={}, auth_plugin={}, scramble_len={}",
                 read_callbacks_->connection(), greeting.getVersion(),
                 greeting.getAuthPluginName(), greeting.getAuthPluginData().size());
  if (config_->terminate_ssl_) {
    server_scramble_ = greeting.getAuthPluginData();
    auth_plugin_name_ = greeting.getAuthPluginName();
    ENVOY_CONN_LOG(trace, "mysql_proxy: captured scramble for SSL termination, plugin={}",
                   read_callbacks_->connection(), auth_plugin_name_);
  }
}
...
void MySQLFilter::onClientLoginResponse(ClientLoginResponse& client_login_resp) {
  ...
  } else if (config_->terminate_ssl_ && client_login_resp.getRespCode() == MYSQL_RESP_MORE) {
...
void MySQLFilter::onAuthSwitchMoreClientData(const std::string& data) {
  ENVOY_CONN_LOG(trace, "mysql_proxy: client auth data received, len={}, rsa_state={}",
                 read_callbacks_->connection(), data.size(), static_cast<int>(rsa_auth_state_));
  if (rsa_auth_state_ == RsaAuthState::WaitingClientPassword) {
    cleartext_password_ = data;
    ENVOY_CONN_LOG(trace, "mysql_proxy: captured cleartext password ({} bytes)",
                   read_callbacks_->connection(), data.size());
  }
}

void MySQLFilter::sendEncryptedPassword(const std::string& pem_key, uint8_t last_server_seq) {
  // XOR password with scramble. The cleartext_password_ includes the trailing null
  // sent by the client (password\0 format for caching_sha2_password).
  std::vector<uint8_t> xored(cleartext_password_.size());
  for (size_t i = 0; i < cleartext_password_.size(); i++) {
    xored[i] = static_cast<uint8_t>(cleartext_password_[i]) ^
               server_scramble_[i % server_scramble_.size()];
  }
  ...
  if (EVP_PKEY_encrypt(ctx.get(), encrypted.data(), &out_len, xored.data(), xored.size()) <= 0) {
    ENVOY_CONN_LOG(error, "mysql_proxy: RSA encryption failed",
                   read_callbacks_->connection());
    read_callbacks_->connection().close(Network::ConnectionCloseType::NoFlush);
    return;
  }
  ...
  ENVOY_CONN_LOG(trace, "mysql_proxy: injecting RSA-encrypted password (seq={}, {} bytes ciphertext)",
                 read_callbacks_->connection(), enc_seq, encrypted.size());

  // Inject toward server (bypasses our filter).
  read_callbacks_->injectReadDataToFilterChain(buf, false);

  cleartext_password_.clear();
}
